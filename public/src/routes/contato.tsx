  ...
  head: () => ({
    meta: [
      { title: "Contato | Felipe Vasconcelos — Corretor em Santarém-PA" },
      {
        name: "description",
        content:
          "Entre em contato com Felipe Vasconcelos. Atendimento via WhatsApp, e-mail e formulário.",
      },
    ],
  }),
...
function ContatoPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Contato registrado. Felipe entrará em contato em breve.");
          ...
          <form onSubmit={onSubmit} className="bg-white p-8 shadow-elegant-sm md:p-12">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <button
                type="submit"
                disabled={submitting}
                className="premium-cta inline-flex items-center gap-2 bg-navy px-7 py-3.5 text-[13px] uppercase tracking-[0.08em] text-white hover:bg-navy-mid disabled:cursor-not-allowed disabled:opacity-60"
              >
                <Send className="h-4 w-4" /> {submitting ? "Enviando..." : "Enviar mensagem"}
              </button>
            </div>
          </form>
          ...
          <aside className="flex flex-col gap-6">
            ...
            <div className="border border-border bg-white p-8">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-4 inline-flex items-center gap-2 text-[13px] uppercase tracking-[0.08em] text-navy hover:text-gold"
              >
                Ver no Google Maps →
              </a>
            </div>
